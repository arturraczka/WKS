...
if test -e ${CERTS_DIR_PATH}/website.crt && test -e ${CERTS_DIR_PATH}/website.key ; then #check if files exists if not create certs
  ...
  cp  ${CERTS_DIR_PATH}/website.crt  ${SCRIPT_PATH_DIR}/website.crt
  cp  ${CERTS_DIR_PATH}/website.key  ${SCRIPT_PATH_DIR}/website.key
else
  echo "create dummy cert for nginx"
  openssl req -x509 -nodes -days 365 -newkey rsa:2048 -subj "/C=PE/ST=Lima/L=Lima/O=Acme Inc. /OU=IT Department/CN=localhost" -keyout ${SCRIPT_PATH_DIR}/website.key -out ${SCRIPT_PATH_DIR}/website.crt
fi
