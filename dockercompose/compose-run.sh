...
if [[ -z "$(${DOCKER_COMPOSE_COMMAND} -f ${COMPOSE_FILE_PATH} ps -q)" ]]; then
  ...
  ${DOCKER_COMPOSE_COMMAND} -f ${COMPOSE_FILE_PATH} up -d
else
  echo "docker compose running is already running"
  echo "RESTART docker compose running"
  ${DOCKER_COMPOSE_COMMAND} -f ${COMPOSE_FILE_PATH} up -d #up when compose.yaml change
  sleep 2 #arbitral sleep helps before restart
  ${DOCKER_COMPOSE_COMMAND} -f ${COMPOSE_FILE_PATH} restart #when compose.yaml does not change but other things must be reinit
fi
